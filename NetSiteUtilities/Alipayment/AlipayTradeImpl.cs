...
namespace EZOper.NetSiteUtilities.Alipayment
{
    ...
    public class AlipayTradeImpl : IAlipayTradeService
    {
        ...
        public AlipayTradeImpl(string serverUrl, string appId, string merchant_private_key, string format, string version, string sign_type, string alipay_public_key, string charset)
        {
            client = new DefaultAopClient(serverUrl, appId, merchant_private_key, format, version, sign_type, alipay_public_key, charset);
        }
        ...
        public AlipayF2FPayResult tradePay(AlipayTradePayContentBuilder builder)
        {
            ...
            try
            {
                ...
                if (payResponse != null)
                {
                    ...
                    {
                        case AlipayResultCode.SUCCESS:
                            break;

                        //返回支付处理中，需要进行轮询
                        case AlipayResultCode.INRROCESS:
                            ...
                            if (queryResponse != null)
                            {
                                if (queryResponse.TradeStatus == "WAIT_BUYER_PAY")
                                {
                                    ...
                                    payResponse.Code = AlipayResultCode.FAIL;
                                }
                ...
                else
                {
                    ...
                    {
                        if (queryResponse3.TradeStatus == AlipayTradeStatus.WAIT_BUYER_PAY)
                        {
                            AlipayTradeCancelResponse cancelResponse = CancelAndRetry(builder.out_trade_no);
                            payResponse.Code = AlipayResultCode.FAIL;
                        }
        ...
        {
            if (queryResponse == null || queryResponse.Code != AlipayResultCode.SUCCESS)
                return null;
            AlipayTradePayResponse payResponse = new AlipayTradePayResponse();

            if (queryResponse.TradeStatus == AlipayTradeStatus.WAIT_BUYER_PAY)
            {
                payResponse.Code = AlipayResultCode.INRROCESS;
            }
            if (queryResponse.TradeStatus == AlipayTradeStatus.TRADE_FINISHED
                || queryResponse.TradeStatus == AlipayTradeStatus.TRADE_SUCCESS)
            {
                payResponse.Code = AlipayResultCode.SUCCESS;
            }
            if (queryResponse.TradeStatus == AlipayTradeStatus.TRADE_CLOSED)
            {
                payResponse.Code = AlipayResultCode.FAIL;
            }
        ...
        private AlipayTradeQueryResponse LoopQuery(string out_trade_no, int count, int interval)
        {
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                if (queryResponse != null && string.Compare(queryResponse.Code, AlipayResultCode.SUCCESS, false) == 0)
                {
        ...
        private AlipayTradeCancelResponse CancelAndRetry(string out_trade_no)
        {
            ...
            if (cancelResponse == null || (cancelResponse.Code == AlipayResultCode.FAIL && cancelResponse.RetryFlag == "Y"))
            {
        ...
        private void cancelThreadFunc(object o)
        {
            ...
            for (int i = 0; i < RETRYCOUNT; ++i)
            {
                ...
                {
                    if (cancelResponse.Code == AlipayResultCode.FAIL)
                    {
                    ...
                    if ((cancelResponse.Code == AlipayResultCode.SUCCESS))
                    {
                        break;
                    }
                }
